...
<table class="form_table">
    ...
    <tr>
        <td class="tb_title"><?php echo $form->label($model, 'apiname'); ?>：</td>
    </tr>
    <tr >
    ...
    <tr >
        <td class="appdata">    	
            ...
            <?php elseif ($model->id == 'weixin'): ?>
                <label class="config_label">App ID(应用id)：</label><input type="text" name="config[app_id]" value="<?php echo $apiconfig['app_id']; ?>" /><br/><br/>
                <label class="config_label">App Secret(应用secret)：</label><input type="text" name="config[app_secret]" value="<?php echo $apiconfig['app_secret']; ?>" size="50" /><br/><br/>
                <label class="config_label">callback(回调地址)：</label><input type="text" name="config[callback]" value="<?php echo $apiconfig['callback']; ?>" size="50"/>(域名/oAuth/weixin_callback)<br/><br/>
                <!-- 其他 -->
            <?php else: ?>
    ...
    <?php if ($model->id == 'qq'): ?>
        <tr>
            <td class="tb_title">
                请求授权列表：<input type="checkbox" name="chkall" id="chkall" onclick="checkAll(this.form, 'scope')" />
