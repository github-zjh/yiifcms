...
<table class="form_table">
    ...
    <tr>
        <td class="tb_title"><?php echo $form->label($model->content, 'cover_img'); ?>：</td>
    </tr>
    <tr>
        <td colspan="2" >
            <?php echo $form->hiddenField($model->content, 'cover_img');?>            
            <input name="simple_file" id="fileupload_cover" onclick="fileUpload()" type="file" />
            <div id="cover_preview">
                <?php if ($model->content->cover_img): ?>        
                <a href="<?php echo Helper::getFullUrl($model->content->cover_img); ?>" target="_blank">
                    <img style="max-width:300px; max-height:300px; padding:5px; border:1px solid #cccccc;" src="<?php echo Helper::getFullUrl($model->content->cover_img); ?>" align="absmiddle" />
                </a>
                <?php endif; ?>
            </div>
        </td>
    ...
    <tr>
        <td><?php echo $form->textArea($model->content, 'content'); ?>      
            <?php $this->widget('application.widget.kindeditor.KindEditor', array('id' => 'SpiderVideoContent_content')); ?>
        </td>
    </tr>
    <?php endif;?>
            ...
            dataType: 'json',
                ...
                if (200 === data.code) {
                    ...
                    if (data.data.file_path) {
                        $('#SpiderVideoContent_cover_img').val(data.data.file_path);
